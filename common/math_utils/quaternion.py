...
def are_equal(q1, q2, rtol=1e-5, atol=1e-8):
    ...
    >>> q1 = [1, 0, 0, 0]
    >>> tfu.quaternion.are_equal(q1, [0, 1, 0, 0])
    False
    >>> tfu.quaternion.are_equal(q1, [1, 0, 0, 0])
    True
    >>> tfu.quaternion.are_equal(q1, [-1, 0, 0, 0])
    True
    """
...
def conjugate(q):
    ...
    --------
    >>> import trifinger_mujoco.utils as tfu
    >>> q0 = tfu.quaternion.random()
    >>> q1 = tfu.quaternion.conjugate(q0)
...
def dual_to_transform(qr, qt):
    ...
    """
    T = np.eye(4)
    R = tfu.quaternion.to_transform(qr)[:3, :3]
    t = 2 * tfu.quaternion.multiply(qt, tfu.quaternion.conjugate(qr))
    T[:3, :3] = R
    T[:3, 3] = t[1:]
    return T
...
def multiply(q1, q2):
    ...
    >>> import numpy as np
    >>> import trifinger_mujoco.utils as tfu
    >>> q = tfu.quaternion.multiply([4, 1, -2, 3], [8, -5, 6, 7])
...
def to_axis_angle(quaternion, identity_thresh=None):
    ...
    if identity_thresh is None:
        try:
            ...
            identity_thresh = tfu._FLOAT_EPS * 3
    if Nq < tfu._FLOAT_EPS**2:  # Results unreliable after normalization
...
def to_euler(quaternion, axes="sxyz"):
    ...
    >>> import numpy as np
    >>> import trifinger_mujoco.utils as tfu
    >>> ai, aj, ak = tfu.quaternion.to_euler([0.99810947, 0.06146124, 0, 0])
    >>> np.allclose([ai, aj, ak], [0.123, 0, 0])
    True
    """
    return tfu.transform.to_euler(to_transform(quaternion), axes)
...
def to_transform(quaternion):
    ...
    >>> import numpy as np
    >>> import trifinger_mujoco.utils as tfu
    >>> T0 = tfu.quaternion.to_transform([1, 0, 0, 0]) # Identity quaternion
    >>> np.allclose(T0, np.eye(4))
    True
    >>> T1 = tfu.quaternion.to_transform([0, 1, 0, 0]) # 180 degree rot around X
