...
def to_euler(transform, axes="sxyz"):
    ...
    >>> import numpy as np
    >>> import trifinger_mujoco.utils as tfu
    >>> T0 = tfu.euler.to_transform(1, 2, 3, 'syxz')
    >>> al, be, ga = tfu.transform.to_euler(T0, 'syxz')
    >>> T1 = tfu.euler.to_transform(al, be, ga, 'syxz')
    >>> np.allclose(T0, T1)
    True
    """
    try:
...
def to_quaternion(transform, isprecise=False):
    ...
    >>> import numpy as np
    >>> import trifinger_mujoco.utils as tfu
    >>> q = tfu.transform.to_quaternion(np.identity(4), isprecise=True)
    >>> np.allclose(q, [1, 0, 0, 0])
    True
    >>> q = tfu.transform.to_quaternion(np.diag([1, -1, -1, 1]))
    >>> np.allclose(q, [0, 1, 0, 0]) or np.allclose(q, [0, -1, 0, 0])
    True
    >>> T = tfu.axis_angle.to_transform((1, 2, 3), 0.123)
    >>> q = tfu.transform.to_quaternion(T, True)
